# Flyway Migration Registry — {{PROJECT_NAME}}
...
|---------|-------|--------|
| **V19** | `V19__Feature_Registration.sql` **and** `V19__Data_Hash.sql` | **DUPLICATE PREFIX** — Flyway may apply unpredictably. Do **not** add another `V19__`. Human renumber/reconcile required before production deploy. |
...
| V18 | `V18__Feature_Setup.sql` |
| V19 | `V19__Feature_Registration.sql` ⚠ |
| V19 | `V19__Data_Hash.sql` ⚠ duplicate |
| V20 | `V20__RLS_Policies.sql` |
| V21 | `V21__Audit_Log.sql` |
| V22 | `V22__Notification_Persistence.sql` |
| V23 | `V23__Approval_Snapshot.sql` |
...
3. **Tenant tables** (workspace-scoped data): add RLS policies — see `V2__Enforce_RLS.sql`, `V20__RLS_Policies.sql`.
